...
if __name__ == "__main__":
    ...
    encoder.load_state_dict(torch.load('src/weights/finetuned_encoder.pth', map_location=device))
    # Load Classifier Head
    class_head = LinearEvaluator(feature_dim=encoder.feature_dim).to(device)
    class_head.load_state_dict(torch.load('src/weights/classifier_head.pth', map_location=device))
    encoder.eval()
    class_head.eval()

    # Load Validation Data
    loader = get_dataloader("src/dataset/validation")
    ...
    pd.DataFrame(results).to_csv('src/validation_prediction.csv', index=False)
